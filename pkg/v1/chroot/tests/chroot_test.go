...
// Same as TestRunChroot but for pivot operations
func TestRunPivot(t *testing.T) {
	// Skip the test if not running as root
	if os.Geteuid() != 0 {
		t.Skip("Skipping test: root privileges required for pivot operations")
	}

	// And also skip if we are inside a container
	if system.RunningInContainer() {
		t.Skip("Skipping test: pivot operations are not supported inside a container")
	}
	...
	// Run a function in the pivot root
	fErr := chroot.RunPivot(alpineRoot, func() error {
		// Check if we are in the new root by reading /etc/os-release
		osReleaseFile := filepath.Join("/etc", "os-release")
		osRelease, err := os.ReadFile(osReleaseFile)
		if err != nil {
	...
	if fErr != nil {
		t.Fatalf("Function in pivot root failed: %v", fErr)
	}

	// Check if we are still in the old root by reading /etc/os-release
	osReleaseFile := filepath.Join(alpineRoot, "/etc", "os-release")
	osRelease, err := os.ReadFile(osReleaseFile)
	if err != nil {
		t.Fatalf("Failed to read /etc/os-release: %v", err)
	}
